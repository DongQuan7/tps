...
///
///  \file   tps_bl_cmd_merge_plan.cpp
///  \brief   
///
///  \version 1.0
///  \date  2015/10/26  
...
//////////////////////////////////////////////////////////////////////////
#include "StdAfx.h"
#include "RtTpsBusinessLogic/tps_bl_cmd_merge_plan.h"
...
MergePlanCmd::MergePlanCmd( CertifiedPlan &primaryPlan, CertifiedPlan &secondaryPlan )
: mPrimaryPlan(primaryPlan), mSencondaryPlan(secondaryPlan), mRegistrationMatrix(nullptr){
	mRegistrationMatrix = new Mcsf::Matrix4f;
}

MergePlanCmd::~MergePlanCmd()
{

}

int MergePlanCmd::Execute()
{
...
bool MergePlanCmd::ValidateImageData_i()
{
...
std::string MergePlanCmd::CreatePlan_i()
{
	...
	//	poiManager->GetPoiUidListByPlanUid(mPrimaryPlan.mPlanUID);
	...
	//	poiManager->GetPoiUidListByPlanUid(mSencondaryPlan.mPlanUID);
...
bool MergePlanCmd::GenerateDoseGrid_i( RtPlan& aPlan )
{
...
RtDosegrid* MergePlanCmd::GetNormgroupDosegrid_i( const std::string& strNgUID )
{
...
bool MergePlanCmd::CreateDoseLineSetting_i( RtPlan* plan )
{
	std::string planUID = plan->get_uid();
	plan->set_specifieddose(0.0);
	plan->set_hundredpercentdefinition(RtDbDef::TotalGoals);
	...
	float ratio = 0.1f, stepRatio = 0.1f;
...
bool MergePlanCmd::CreateNormalGroup_i( const std::string &originalPlanUID, const std::string &mergePlan/*, std::vector<std::string> *mergePlanList*/ )
{
...
RtDosegrid* MergePlanCmd::GetPlanDosegrid_i( const std::string& planUID )
{
	TpsPatientManager *patientManager = mDataRepository->GetPatientManager();
...
std::string MergePlanCmd::GetMergedPlanUID()
{
