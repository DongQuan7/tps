...
///
///  \file      rt_tps_database_object_helper_goldenstt.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtGoldenstt& src, bool bInsert)
{
    ...
    if (!bInsert)
    {
        for (unsigned int i(0); i < GOLDENSTT_FIELD_MAX; ++i)
        {
            if (!src.is_dirty(i)) continue;
            const char* file_name = ENUM2STRING_GOLDENSTT_FIELD[i];
            if (sName.empty())
            {
        ...
        sSql = "UPDATE tmsgoldenstt SET " + sName + " WHERE uid='" + src.get_uid() + "'";
    }
    else
    {
        std::string sValues("");
        ...
        sSql = "INSERT INTO tmsgoldenstt (" + sName + ") VALUES (" + sValues + ")";
    }
    ...
    float		data_wedgefactor = 0.f;
    float		data_wedgeangle = 0.f;
    float		data_fixjawpos = 0.f;
    std::string	data_accumulatedmu = "";
    std::string	data_correctionfactorpencilbeam = "";
    std::string	data_correctionfactorconvolution = "";
    std::string	data_correctionfactormontecarlo = "";
    std::string	data_movjawpos = "";
    std::string	data_exppara = "";
    std::string	data_crc = "";
    ...
    for (unsigned int i(0); i < GOLDENSTT_FIELD_MAX; ++i)
    {
        ...
        case 2: 	//commissionedunituid varchar
            ...
            break;
        case 3: 	//wedgefactor float
            {
                data_wedgefactor = src.get_wedgefactor();
                bind[iCount].buffer_type = MYSQL_TYPE_FLOAT;
                bind[iCount].buffer = (void*)&data_wedgefactor;
                bind[iCount].buffer_length = sizeof(float);
            }
            break;
        case 4: 	//wedgeangle float
            {
            ...
            break;
        case 5: 	//fixjawpos float
            {
                data_fixjawpos = src.get_fixjawpos();
                bind[iCount].buffer_type = MYSQL_TYPE_FLOAT;
                bind[iCount].buffer = (void*)&data_fixjawpos;
                bind[iCount].buffer_length = sizeof(float);
            }
            break;
        case 6: 	//accumulatedmu varchar
            {
                data_accumulatedmu = src.get_accumulatedmu();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_accumulatedmu.c_str();
                bind[iCount].buffer_length = (unsigned long)data_accumulatedmu.size();
            }
            break;
        case 7: 	//correctionfactorpencilbeam varchar
            {
                data_correctionfactorpencilbeam = src.get_correctionfactorpencilbeam();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_correctionfactorpencilbeam.c_str();
                bind[iCount].buffer_length = (unsigned long)data_correctionfactorpencilbeam.size();
            }
            break;
        case 8: 	//correctionfactorconvolution varchar
            {
                data_correctionfactorconvolution = src.get_correctionfactorconvolution();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_correctionfactorconvolution.c_str();
                bind[iCount].buffer_length = (unsigned long)data_correctionfactorconvolution.size();
            }
            break;
        case 9: 	//correctionfactormontecarlo varchar
            {
                data_correctionfactormontecarlo = src.get_correctionfactormontecarlo();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_correctionfactormontecarlo.c_str();
                bind[iCount].buffer_length = (unsigned long)data_correctionfactormontecarlo.size();
            }
            break;
        case 10: 	//movjawpos varchar
            {
                data_movjawpos = src.get_movjawpos();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_movjawpos.c_str();
                bind[iCount].buffer_length = (unsigned long)data_movjawpos.size();
            }
            break;
        case 11: 	//exppara varchar
            {
                data_exppara = src.get_exppara();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_exppara.c_str();
                bind[iCount].buffer_length = (unsigned long)data_exppara.size();
            }
            break;
        case 12: 	//crc varchar
            {
	...
	DEL_ARRAY(bind);
	RtDatabaseObject* rtDatabaseObject = const_cast<RtGoldenstt*>(&src);
	rtDatabaseObject->setdirty_all_fields(false);
    return true;
}

bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtGoldenstt*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt)
    {
    ...
    //tmsgoldenstt    14 parameters
    const int field_num = GOLDENSTT_FIELD_MAX;
    ...
    for (unsigned int i(0); i<column_count; ++i)
    {
        ...
        case 3: 	//wedgefactor float
            ...
            bind[i].buffer= (char *)&data_wedgefactor;
            break;
        case 4: 	//wedgeangle float
            bind[i].buffer_type= MYSQL_TYPE_FLOAT;
        ...
        case 5: 	//fixjawpos float
            bind[i].buffer_type= MYSQL_TYPE_FLOAT;
            bind[i].buffer= (char *)&data_fixjawpos;
            break;
        case 13: 	//updatetime timestamp
    ...
    // Fetch all rows 
    int ret(1);
    RtGoldenstt* pDis = nullptr;
    while(true)
    {
        ret = mysql_stmt_fetch(stmt);
        if (ret!=0 && ret!=MYSQL_DATA_TRUNCATED) break;
        pDis = new RtGoldenstt();
        for (unsigned int i(0); i<column_count; ++i)
        {
            ...
            case 2: 	//commissionedunituid varchar
                ...
                break;
            case 3: 	//wedgefactor float
                pDis->set_wedgefactor(data_wedgefactor);
                break;
            case 4: 	//wedgeangle float
                pDis->set_wedgeangle(data_wedgeangle);
                break;
            case 5: 	//fixjawpos float
                pDis->set_fixjawpos(data_fixjawpos);
                break;
            case 6: 	//accumulatedmu varchar
                pDis->set_accumulatedmu((char *)bind[i].buffer);
                break;
            case 7: 	//correctionfactorpencilbeam varchar
                pDis->set_correctionfactorpencilbeam((char *)bind[i].buffer);
                break;
            case 8: 	//correctionfactorconvolution varchar
                pDis->set_correctionfactorconvolution((char *)bind[i].buffer);
                break;
            case 9: 	//correctionfactormontecarlo varchar
                pDis->set_correctionfactormontecarlo((char *)bind[i].buffer);
                break;
            case 10: 	//movjawpos varchar
                pDis->set_movjawpos((char *)bind[i].buffer);
                break;
            case 11: 	//exppara varchar
                pDis->set_exppara((char *)bind[i].buffer);
                break;
            case 12: 	//crc varchar
