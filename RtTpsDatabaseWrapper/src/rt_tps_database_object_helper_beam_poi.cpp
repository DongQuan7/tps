...
///
///  \file      rt_tps_database_object_helper_beam_poi.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtBeam_poi& src, bool bInsert)
{
    ...
    std::string sSql = "INSERT INTO tmsbeam_poi VALUES(?, ?, ?, ?)";
    if (!bInsert){
        field_num -= 2;
        field_offset = 2;
        sSql = "UPDATE tmsbeam_poi SET "
            "dosecontribution=?,"
            "updatetime=?";
        sSql += " WHERE beamuid='" + src.get_beamuid() + "' AND poiuid='" + src.get_poiuid() + "'";
    }
...
bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtBeam_poi*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt){
        TPS_LOG_DEV_ERROR<<"mysql_stmt_init(), out of memory";
