...
///
///  \file      rt_tps_database_object_helper_tray.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtTray& src, bool bInsert)
{
    ...
    if (!bInsert)
    {
        for (unsigned int i(0); i < TRAY_FIELD_MAX; ++i)
        {
            if (!src.is_dirty(i)) continue;
            const char* file_name = ENUM2STRING_TRAY_FIELD[i];
            if (sName.empty())
            {
        ...
        sSql = "UPDATE tmstray SET " + sName + " WHERE uid='" + src.get_uid() + "'";
    }
    else
    {
        std::string sValues("");
        ...
        sSql = "INSERT INTO tmstray (" + sName + ") VALUES (" + sValues + ")";
    }
    ...
    for (unsigned int i(0); i < TRAY_FIELD_MAX; ++i)
    {
        ...
        case 1: 	//machineuid varchar
            ...
            break;
        case 2: 	//trayid varchar
            {
                data_trayid = src.get_trayid();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_trayid.c_str();
                bind[iCount].buffer_length = (unsigned long)data_trayid.size();
            }
            break;
        case 3: 	//code varchar
            {
	...
	DEL_ARRAY(bind);
	RtDatabaseObject* rtDatabaseObject = const_cast<RtTray*>(&src);
	rtDatabaseObject->setdirty_all_fields(false);
    return true;
}

bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtTray*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt)
    {
    ...
    //tmstray    5 parameters
    const int field_num = TRAY_FIELD_MAX;
    ...
    while(true)
    {
        ...
        for (unsigned int i(0); i<column_count; ++i)
        {
            ...
            case 1: 	//machineuid varchar
                ...
                break;
            case 2: 	//trayid varchar
                pDis->set_trayid((char *)bind[i].buffer);
                break;
            case 3: 	//code varchar
