...
///
///  \file      rt_tps_database_object_helper_drrsetting.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtDrrsetting& src, bool bInsert)
{
    ...
    if (!bInsert)
    {
        for (unsigned int i(0); i < DRRSETTING_FIELD_MAX; ++i)
        {
            if (!src.is_dirty(i)) continue;
            const char* file_name = ENUM2STRING_DRRSETTING_FIELD[i];
            if (sName.empty())
            {
        ...
        sSql = "UPDATE tmsdrrsetting SET " + sName + " WHERE uid='" + src.get_uid() + "'";
    }
    else
    {
        std::string sValues("");
        ...
        sSql = "INSERT INTO tmsdrrsetting (" + sName + ") VALUES (" + sValues + ")";
    }
    ...
    my_bool     isnull = true;
    std::string	data_uid = "";
    std::string	data_normalgroupuid = "";
    float		data_electronenergy = 0.f;
    std::string	data_ct2densityuid = "";
    float		data_ctthresholdminvalue = 0.f;
    float		data_ctthresholdmaxvalue = 0.f;
    ...
    for (unsigned int i(0); i < DRRSETTING_FIELD_MAX; ++i)
    {
        ...
        case 0: 	//uid varchar
            ...
            break;
        case 1: 	//normalgroupuid varchar
            {
                data_normalgroupuid = src.get_normalgroupuid();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_normalgroupuid.c_str();
                bind[iCount].buffer_length = (unsigned long)data_normalgroupuid.size();
            }
            break;
        case 2: 	//electronenergy float
            {
                data_electronenergy = src.get_electronenergy();
                bind[iCount].buffer_type = MYSQL_TYPE_FLOAT;
                bind[iCount].buffer = (void*)&data_electronenergy;
                bind[iCount].buffer_length = sizeof(float);
            }
            break;
        case 3: 	//ct2densityuid varchar
            {
            ...
            break;
        case 4: 	//ctthresholdminvalue float
            {
                data_ctthresholdminvalue = src.get_ctthresholdminvalue();
                bind[iCount].buffer_type = MYSQL_TYPE_FLOAT;
                bind[iCount].buffer = (void*)&data_ctthresholdminvalue;
                bind[iCount].buffer_length = sizeof(float);
            }
            break;
        case 5: 	//ctthresholdmaxvalue float
            {
                data_ctthresholdmaxvalue = src.get_ctthresholdmaxvalue();
                bind[iCount].buffer_type = MYSQL_TYPE_FLOAT;
                bind[iCount].buffer = (void*)&data_ctthresholdmaxvalue;
                bind[iCount].buffer_length = sizeof(float);
            }
            break;
	...
	DEL_ARRAY(bind);
	RtDatabaseObject* rtDatabaseObject = const_cast<RtDrrsetting*>(&src);
	rtDatabaseObject->setdirty_all_fields(false);
    return true;
}

bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtDrrsetting*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt)
    {
    ...
    //tmsdrrsetting    7 parameters
    const int field_num = DRRSETTING_FIELD_MAX;
    ...
    for (unsigned int i(0); i<column_count; ++i)
    {
        ...
        case 2: 	//electronenergy float
            ...
            bind[i].buffer= (char *)&data_electronenergy;
            break;
        case 4: 	//ctthresholdminvalue float
            bind[i].buffer_type= MYSQL_TYPE_FLOAT;
            bind[i].buffer= (char *)&data_ctthresholdminvalue;
            break;
        case 5: 	//ctthresholdmaxvalue float
            bind[i].buffer_type= MYSQL_TYPE_FLOAT;
            bind[i].buffer= (char *)&data_ctthresholdmaxvalue;
            break;
        case 6: 	//updatetime timestamp
    ...
    // Fetch all rows 
    int ret(1);
    RtDrrsetting* pDis = nullptr;
    while(true)
    {
        ret = mysql_stmt_fetch(stmt);
        if (ret!=0 && ret!=MYSQL_DATA_TRUNCATED) break;
        pDis = new RtDrrsetting();
        for (unsigned int i(0); i<column_count; ++i)
        {
            ...
            case 0: 	//uid varchar
                ...
                break;
            case 1: 	//normalgroupuid varchar
                pDis->set_normalgroupuid((char *)bind[i].buffer);
                break;
            case 2: 	//electronenergy float
                pDis->set_electronenergy(data_electronenergy);
                break;
            case 3: 	//ct2densityuid varchar
                pDis->set_ct2densityuid((char *)bind[i].buffer);
                break;
            case 4: 	//ctthresholdminvalue float
                pDis->set_ctthresholdminvalue(data_ctthresholdminvalue);
                break;
            case 5: 	//ctthresholdmaxvalue float
                pDis->set_ctthresholdmaxvalue(data_ctthresholdmaxvalue);
                break;
            case 6: 	//updatetime timestamp
                {
