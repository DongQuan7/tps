...
///
///  \file      rt_tps_database_object_helper_seriesgroup.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtSeriesgroup& src, bool bInsert)
{
    ...
    if (!bInsert)
    {
        for (unsigned int i(0); i < SERIESGROUP_FIELD_MAX; ++i)
        {
            if (!src.is_dirty(i)) continue;
            const char* file_name = ENUM2STRING_SERIESGROUP_FIELD[i];
            if (sName.empty())
            {
        ...
        sSql = "UPDATE tmsseriesgroup SET " + sName + " WHERE uid='" + src.get_uid() + "'";
    }
    else
    {
        std::string sValues("");
        ...
        sSql = "INSERT INTO tmsseriesgroup (" + sName + ") VALUES (" + sValues + ")";
    }
    ...
    int			data_grouptype = 0;
    std::string	data_mainseriesuid = "";
    ...
    for (unsigned int i(0); i < SERIESGROUP_FIELD_MAX; ++i)
    {
        ...
        case 2: 	//name varchar
            ...
            break;
        case 3: 	//grouptype int
            {
                data_grouptype = src.get_grouptype();
                bind[iCount].buffer_type = MYSQL_TYPE_LONG;
                bind[iCount].buffer = (void*)&data_grouptype;
                bind[iCount].buffer_length = sizeof(int);
            }
            break;
        case 4: 	//mainseriesuid varchar
            {
                data_mainseriesuid = src.get_mainseriesuid();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_mainseriesuid.c_str();
                bind[iCount].buffer_length = (unsigned long)data_mainseriesuid.size();
            }
            break;
	...
	DEL_ARRAY(bind);
	RtDatabaseObject* rtDatabaseObject = const_cast<RtSeriesgroup*>(&src);
	rtDatabaseObject->setdirty_all_fields(false);
    return true;
}

bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtSeriesgroup*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt)
    {
    ...
    //tmsseriesgroup    6 parameters
    const int field_num = SERIESGROUP_FIELD_MAX;
    ...
    for (unsigned int i(0); i<column_count; ++i)
    {
        ...
        case 3: 	//grouptype int
            ...
            bind[i].buffer= (char *)&data_grouptype;
            break;
        case 5: 	//updatetime timestamp
    ...
    // Fetch all rows 
    int ret(1);
    RtSeriesgroup* pDis = nullptr;
    while(true)
    {
        ret = mysql_stmt_fetch(stmt);
        if (ret!=0 && ret!=MYSQL_DATA_TRUNCATED) break;
        pDis = new RtSeriesgroup();
        for (unsigned int i(0); i<column_count; ++i)
        {
            ...
            case 2: 	//name varchar
                ...
                break;
            case 3: 	//grouptype int
                pDis->set_grouptype(data_grouptype);
                break;
            case 4: 	//mainseriesuid varchar
                pDis->set_mainseriesuid((char *)bind[i].buffer);
                break;
            case 5: 	//updatetime timestamp
                {
