...
///
///  \file      rt_tps_database_object_helper_kerneldatalist.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtKerneldatalist& src, bool bInsert)
{
    ...
    if (!bInsert)
    {
        for (unsigned int i(0); i < KERNELDATALIST_FIELD_MAX; ++i)
        {
            if (!src.is_dirty(i)) continue;
            const char* file_name = ENUM2STRING_KERNELDATALIST_FIELD[i];
            if (sName.empty())
            {
        ...
        sSql = "UPDATE tmskerneldatalist SET " + sName + " WHERE uid='" + src.get_uid() + "'";
    }
    else
    {
        std::string sValues("");
        ...
        sSql = "INSERT INTO tmskerneldatalist (" + sName + ") VALUES (" + sValues + ")";
    }
    ...
    std::string	data_uid = "";
    int			data_numofphi = 0;
    std::string	data_phidata = "";
    int			data_numofr = 0;
    std::string	data_rdata = "";
    std::string	data_crc = "";
    ...
    for (unsigned int i(0); i < KERNELDATALIST_FIELD_MAX; ++i)
    {
        ...
        case 1: 	//numofphi int
            {
                data_numofphi = src.get_numofphi();
                bind[iCount].buffer_type = MYSQL_TYPE_LONG;
                bind[iCount].buffer = (void*)&data_numofphi;
                bind[iCount].buffer_length = sizeof(int);
            }
            break;
        case 2: 	//phidata varchar
            {
                data_phidata = src.get_phidata();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_phidata.c_str();
                bind[iCount].buffer_length = (unsigned long)data_phidata.size();
            }
            break;
        case 3: 	//numofr int
            {
                data_numofr = src.get_numofr();
                bind[iCount].buffer_type = MYSQL_TYPE_LONG;
                bind[iCount].buffer = (void*)&data_numofr;
                bind[iCount].buffer_length = sizeof(int);
            }
            break;
        case 4: 	//rdata varchar
            {
                data_rdata = src.get_rdata();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_rdata.c_str();
                bind[iCount].buffer_length = (unsigned long)data_rdata.size();
            }
            break;
        case 5: 	//crc varchar
            {
	...
	DEL_ARRAY(bind);
	RtDatabaseObject* rtDatabaseObject = const_cast<RtKerneldatalist*>(&src);
	rtDatabaseObject->setdirty_all_fields(false);
    return true;
}

bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtKerneldatalist*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt)
    {
    ...
    //tmskerneldatalist    7 parameters
    const int field_num = KERNELDATALIST_FIELD_MAX;
    ...
    for (unsigned int i(0); i<column_count; ++i)
    {
        ...
        case 1: 	//numofphi int
            ...
            bind[i].buffer= (char *)&data_numofphi;
            break;
        case 3: 	//numofr int
            bind[i].buffer_type= MYSQL_TYPE_LONG;
            bind[i].buffer= (char *)&data_numofr;
            break;
        case 6: 	//updatetime timestamp
    ...
    // Fetch all rows 
    int ret(1);
    RtKerneldatalist* pDis = nullptr;
    while(true)
    {
        ret = mysql_stmt_fetch(stmt);
        if (ret!=0 && ret!=MYSQL_DATA_TRUNCATED) break;
        pDis = new RtKerneldatalist();
        for (unsigned int i(0); i<column_count; ++i)
        {
            ...
            case 0: 	//uid varchar
                ...
                break;
            case 1: 	//numofphi int
                pDis->set_numofphi(data_numofphi);
                break;
            case 2: 	//phidata varchar
                pDis->set_phidata((char *)bind[i].buffer);
                break;
            case 3: 	//numofr int
                pDis->set_numofr(data_numofr);
                break;
            case 4: 	//rdata varchar
                pDis->set_rdata((char *)bind[i].buffer);
                break;
            case 5: 	//crc varchar
