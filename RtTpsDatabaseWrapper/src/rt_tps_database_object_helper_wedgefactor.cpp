...
///
///  \file      rt_tps_database_object_helper_wedgefactor.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtWedgefactor& src, bool bInsert)
{
    ...
    if (!bInsert)
    {
        for (unsigned int i(0); i < WEDGEFACTOR_FIELD_MAX; ++i)
        {
            if (!src.is_dirty(i)) continue;
            const char* file_name = ENUM2STRING_WEDGEFACTOR_FIELD[i];
            if (sName.empty())
            {
        ...
        sSql = "UPDATE tmswedgefactor SET " + sName + " WHERE uid='" + src.get_uid() + "'";
    }
    else
    {
        std::string sValues("");
        ...
        sSql = "INSERT INTO tmswedgefactor (" + sName + ") VALUES (" + sValues + ")";
    }
    ...
    std::string	data_wedgefactorlist = "";
    std::string	data_crc = "";
    ...
    for (unsigned int i(0); i < WEDGEFACTOR_FIELD_MAX; ++i)
    {
        ...
        case 1: 	//commissionedunituid varchar
            ...
            break;
        case 2: 	//wedgefactorlist varchar
            {
                data_wedgefactorlist = src.get_wedgefactorlist();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_wedgefactorlist.c_str();
                bind[iCount].buffer_length = (unsigned long)data_wedgefactorlist.size();
            }
            break;
        case 3: 	//crc varchar
            {
	...
	DEL_ARRAY(bind);
	RtDatabaseObject* rtDatabaseObject = const_cast<RtWedgefactor*>(&src);
	rtDatabaseObject->setdirty_all_fields(false);
    return true;
}

bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtWedgefactor*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt)
    {
    ...
    //tmswedgefactor    5 parameters
    const int field_num = WEDGEFACTOR_FIELD_MAX;
    ...
    // Fetch all rows 
    int ret(1);
    RtWedgefactor* pDis = nullptr;
    while(true)
    {
        ret = mysql_stmt_fetch(stmt);
        if (ret!=0 && ret!=MYSQL_DATA_TRUNCATED) break;
        pDis = new RtWedgefactor();
        for (unsigned int i(0); i<column_count; ++i)
        {
            ...
            case 1: 	//commissionedunituid varchar
                ...
                break;
            case 2: 	//wedgefactorlist varchar
                pDis->set_wedgefactorlist((char *)bind[i].buffer);
                break;
            case 3: 	//crc varchar
