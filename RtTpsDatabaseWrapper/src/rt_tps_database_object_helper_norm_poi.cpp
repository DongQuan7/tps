...
///
///  \file      rt_tps_database_object_helper_norm_poi.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtNorm_poi& src, bool bInsert)
{
    ...
    std::string sSql = "INSERT INTO tmsnorm_poi VALUES(?, ?, ?, ?)";
    if (!bInsert){
        field_num -= 2;
        field_offset = 2;
        sSql = "UPDATE tmsnorm_poi SET "
            "dosecontribution=?,"
            "updatetime=?";
        sSql += " WHERE normgroupuid='" + src.get_normgroupuid() + "' AND poiuid='" + src.get_poiuid() + "'";
    }
    ...
    std::string	data_poiuid = "";
    float		data_dosecontribution = 0.f;
    MYSQL_TIME	data_updatetime;

    for (unsigned int i(0); i<field_num; ++i){
        switch (i + field_offset){
        case 0: 	//normgroupuid varchar
            {
                data_normgroupuid = src.get_normgroupuid();
                bind[i].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[i].buffer = (void*)data_normgroupuid.c_str();
                bind[i].buffer_length = (unsigned long)data_normgroupuid.size();
                if(data_normgroupuid.empty()) bind[i].is_null = &isnull;
            }
            break;
        case 1: 	//poiuid varchar
            {
                data_poiuid = src.get_poiuid();
                bind[i].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[i].buffer = (void*)data_poiuid.c_str();
                bind[i].buffer_length = (unsigned long)data_poiuid.size();
                if(data_poiuid.empty()) bind[i].is_null = &isnull;
            }
            break;
        case 2: 	//dosecontribution float
            {
                data_dosecontribution = src.get_dosecontribution();
                bind[i].buffer_type = MYSQL_TYPE_FLOAT;
                bind[i].buffer = (void*)&data_dosecontribution;
...
bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtNorm_poi*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt){
        TPS_LOG_DEV_ERROR<<"mysql_stmt_init(), out of memory";
    ...
    for (unsigned int i(0); i<column_count; ++i){
        ...
        case 2: 	//dosecontribution float
            bind[i].buffer_type= MYSQL_TYPE_FLOAT;
            bind[i].buffer= (char *)&data_dosecontribution;
            break;
        case 3: 	//updatetime timestamp
            bind[i].buffer_type= MYSQL_TYPE_TIMESTAMP;
    ...
    while(true){
        ...
        for (unsigned int i(0); i<column_count; ++i){
            ...
            case 0: 	//normgroupuid varchar
                ...
                break;
            case 1: 	//poiuid varchar
                pDis->set_poiuid((char *)bind[i].buffer);
                break;
            case 2: 	//dosecontribution float
                pDis->set_dosecontribution(data_dosecontribution);
                break;
            case 3: 	//updatetime timestamp
                {
