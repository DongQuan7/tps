...
///
///  \file      rt_tps_database_object_helper_mcphomonoenergyweight.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtMcphomonoenergyweight& src, bool bInsert)
{
    ...
    if (!bInsert)
    {
        for (unsigned int i(0); i < MCPHOMONOENERGYWEIGHT_FIELD_MAX; ++i)
        {
            if (!src.is_dirty(i)) continue;
            const char* file_name = ENUM2STRING_MCPHOMONOENERGYWEIGHT_FIELD[i];
            if (sName.empty())
            {
        ...
        sSql = "UPDATE tmsmcphomonoenergyweight SET " + sName + " WHERE uid='" + src.get_uid() + "'";
    }
    else
    {
        std::string sValues("");
        ...
        sSql = "INSERT INTO tmsmcphomonoenergyweight (" + sName + ") VALUES (" + sValues + ")";
    }
    ...
    my_bool     isnull = true;
    std::string	data_uid = "";
    std::string	data_mcphocommissionedunituid = "";
    float		data_injectelectronenergy = 0.f;
    float		data_injectelectronweight = 0.f;
    float		data_phspdistancefromtarget = 0.f;
    std::string	data_filepath = "";
    std::string	data_phspfilepath = "";
    std::string	data_crc = "";
    ...
    for (unsigned int i(0); i < MCPHOMONOENERGYWEIGHT_FIELD_MAX; ++i)
    {
        ...
        case 0: 	//uid varchar
            ...
            break;
        case 1: 	//mcphocommissionedunituid varchar
            {
                data_mcphocommissionedunituid = src.get_mcphocommissionedunituid();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_mcphocommissionedunituid.c_str();
                bind[iCount].buffer_length = (unsigned long)data_mcphocommissionedunituid.size();
            }
            break;
        case 2: 	//injectelectronenergy float
            {
                data_injectelectronenergy = src.get_injectelectronenergy();
                bind[iCount].buffer_type = MYSQL_TYPE_FLOAT;
                bind[iCount].buffer = (void*)&data_injectelectronenergy;
                bind[iCount].buffer_length = sizeof(float);
            }
            break;
        case 3: 	//injectelectronweight float
            {
                data_injectelectronweight = src.get_injectelectronweight();
                bind[iCount].buffer_type = MYSQL_TYPE_FLOAT;
                bind[iCount].buffer = (void*)&data_injectelectronweight;
                bind[iCount].buffer_length = sizeof(float);
            }
            break;
        case 4: 	//phspdistancefromtarget float
            {
            ...
            break;
        case 5: 	//filepath varchar
            {
                data_filepath = src.get_filepath();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_filepath.c_str();
                bind[iCount].buffer_length = (unsigned long)data_filepath.size();
            }
            break;
        case 6: 	//phspfilepath varchar
            {
                data_phspfilepath = src.get_phspfilepath();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_phspfilepath.c_str();
                bind[iCount].buffer_length = (unsigned long)data_phspfilepath.size();
            }
            break;
        case 7: 	//crc varchar
            {
	...
	DEL_ARRAY(bind);
	RtDatabaseObject* rtDatabaseObject = const_cast<RtMcphomonoenergyweight*>(&src);
	rtDatabaseObject->setdirty_all_fields(false);
    return true;
}

bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtMcphomonoenergyweight*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt)
    {
    ...
    //tmsmcphomonoenergyweight    9 parameters
    const int field_num = MCPHOMONOENERGYWEIGHT_FIELD_MAX;
    ...
    for (unsigned int i(0); i<column_count; ++i)
    {
        ...
        case 2: 	//injectelectronenergy float
            ...
            bind[i].buffer= (char *)&data_injectelectronenergy;
            break;
        case 3: 	//injectelectronweight float
            bind[i].buffer_type= MYSQL_TYPE_FLOAT;
            bind[i].buffer= (char *)&data_injectelectronweight;
            break;
        case 4: 	//phspdistancefromtarget float
            bind[i].buffer_type= MYSQL_TYPE_FLOAT;
    ...
    // Fetch all rows 
    int ret(1);
    RtMcphomonoenergyweight* pDis = nullptr;
    while(true)
    {
        ret = mysql_stmt_fetch(stmt);
        if (ret!=0 && ret!=MYSQL_DATA_TRUNCATED) break;
        pDis = new RtMcphomonoenergyweight();
        for (unsigned int i(0); i<column_count; ++i)
        {
            ...
            case 0: 	//uid varchar
                ...
                break;
            case 1: 	//mcphocommissionedunituid varchar
                pDis->set_mcphocommissionedunituid((char *)bind[i].buffer);
                break;
            case 2: 	//injectelectronenergy float
                pDis->set_injectelectronenergy(data_injectelectronenergy);
                break;
            case 3: 	//injectelectronweight float
                pDis->set_injectelectronweight(data_injectelectronweight);
                break;
            case 4: 	//phspdistancefromtarget float
                pDis->set_phspdistancefromtarget(data_phspdistancefromtarget);
                break;
            case 5: 	//filepath varchar
                pDis->set_filepath((char *)bind[i].buffer);
                break;
            case 6: 	//phspfilepath varchar
                pDis->set_phspfilepath((char *)bind[i].buffer);
                break;
            case 7: 	//crc varchar
