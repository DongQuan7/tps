...
///
///  \file      rt_tps_database_object_helper_norm_voi.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtNorm_voi& src, bool bInsert)
{
    ...
    std::string sSql = "INSERT INTO tmsnorm_voi VALUES(?, ?, ?, ?, ?, ?"
        ", ?)";
    if (!bInsert){
        field_num -= 2;
        field_offset = 2;
        sSql = "UPDATE tmsnorm_voi SET "
            //"uid,"
            "mindosecontribution=?,"
            ...
            "iscare=?,"
            "updatetime=?";
        sSql += " WHERE normgroupuid='" + src.get_normgroupuid() + "' AND voiuid='" + src.get_voiuid() + "'";
    }
    ...
    for (unsigned int i(0); i<field_num; ++i){
        ...
        case 4: 	//maxdosecontribution float
            ...
            break;
        case 5: 	//iscare tinyint
            {
                data_iscare = src.get_iscare();
                bind[i].buffer_type = MYSQL_TYPE_TINY;
                bind[i].buffer = (void*)&data_iscare;
                bind[i].buffer_length = sizeof(bool);
            }
            break;
...
bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtNorm_voi*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt){
        TPS_LOG_DEV_ERROR<<"mysql_stmt_init(), out of memory";
    ...
    for (unsigned int i(0); i<column_count; ++i){
        ...
        case 5: 	//iscare tinyint
            bind[i].buffer_type= MYSQL_TYPE_TINY;
            bind[i].buffer= (char *)&data_iscare;
            break;
        case 6: 	//updatetime timestamp
            bind[i].buffer_type= MYSQL_TYPE_TIMESTAMP;
