...
///
///  \file      rt_tps_database_object_helper_template.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtTemplate& src, bool bInsert)
{
    ...
    if (!bInsert)
    {
        for (unsigned int i(0); i < TEMPLATE_FIELD_MAX; ++i)
        {
            if (!src.is_dirty(i)) continue;
            const char* file_name = ENUM2STRING_TEMPLATE_FIELD[i];
            if (sName.empty())
            {
        ...
        sSql = "UPDATE tmstemplate SET " + sName + " WHERE uid='" + src.get_uid() + "'";
    }
    else
    {
        std::string sValues("");
        ...
        sSql = "INSERT INTO tmstemplate (" + sName + ") VALUES (" + sValues + ")";
    }
    ...
    for (unsigned int i(0); i < TEMPLATE_FIELD_MAX; ++i)
    {
        ...
        case 1: 	//name varchar
            ...
            break;
        case 2: 	//value varchar
            {
                data_value = src.get_value();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_value.c_str();
                bind[iCount].buffer_length = (unsigned long)data_value.size();
            }
            break;
        case 3: 	//datatype int
            {
	...
	DEL_ARRAY(bind);
	RtDatabaseObject* rtDatabaseObject = const_cast<RtTemplate*>(&src);
	rtDatabaseObject->setdirty_all_fields(false);
    return true;
}

bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtTemplate*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt)
    {
    ...
    //tmstemplate    5 parameters
    const int field_num = TEMPLATE_FIELD_MAX;
    ...
    while(true)
    {
        ...
        for (unsigned int i(0); i<column_count; ++i)
        {
            ...
            case 1: 	//name varchar
                ...
                break;
            case 2: 	//value varchar
                pDis->set_value((char *)bind[i].buffer);
                break;
            case 3: 	//datatype int
