...
///
///  \file      rt_tps_database_object_helper_tcp.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtTcp& src, bool bInsert)
{
    ...
    if (!bInsert)
    {
        for (unsigned int i(0); i < TCP_FIELD_MAX; ++i)
        {
            if (!src.is_dirty(i)) continue;
            const char* file_name = ENUM2STRING_TCP_FIELD[i];
            if (sName.empty())
            {
        ...
        sSql = "UPDATE tmstcp SET " + sName + " WHERE uid='" + src.get_uid() + "'";
    }
    else
    {
        std::string sValues("");
        ...
        sSql = "INSERT INTO tmstcp (" + sName + ") VALUES (" + sValues + ")";
    }
    ...
    for (unsigned int i(0); i < TCP_FIELD_MAX; ++i)
    {
        ...
        case 5: 	//gamma float
            {
                data_gamma = src.get_gamma();
                bind[iCount].buffer_type = MYSQL_TYPE_FLOAT;
                bind[iCount].buffer = (void*)&data_gamma;
        ...
        case 7: 	//e float
            {
                data_e = src.get_e();
                bind[iCount].buffer_type = MYSQL_TYPE_FLOAT;
                bind[iCount].buffer = (void*)&data_e;
                bind[iCount].buffer_length = sizeof(float);
            }
            break;
        case 8: 	//m float
	...
	DEL_ARRAY(bind);
	RtDatabaseObject* rtDatabaseObject = const_cast<RtTcp*>(&src);
	rtDatabaseObject->setdirty_all_fields(false);
    return true;
}

bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtTcp*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt)
    {
    ...
    //tmstcp    11 parameters
    const int field_num = TCP_FIELD_MAX;
