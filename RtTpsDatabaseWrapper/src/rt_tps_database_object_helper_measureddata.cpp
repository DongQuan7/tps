...
///
///  \file      rt_tps_database_object_helper_measureddata.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtMeasureddata& src, bool bInsert)
{
    ...
    if (!bInsert)
    {
        for (unsigned int i(0); i < MEASUREDDATA_FIELD_MAX; ++i)
        {
            if (!src.is_dirty(i)) continue;
            const char* file_name = ENUM2STRING_MEASUREDDATA_FIELD[i];
            if (sName.empty())
            {
        ...
        sSql = "UPDATE tmsmeasureddata SET " + sName + " WHERE uid='" + src.get_uid() + "'";
    }
    else
    {
        std::string sValues("");
        ...
        sSql = "INSERT INTO tmsmeasureddata (" + sName + ") VALUES (" + sValues + ")";
    }
    ...
    std::string	data_outputinwaterlist = "";
    std::string	data_outputinairlist = "";
    float		data_phantomagle = 0.f;
    float		data_scpdepth = 0.f;
    float		data_wedgefactordepth = 0.f;
    std::string	data_crc = "";
    ...
    for (unsigned int i(0); i < MEASUREDDATA_FIELD_MAX; ++i)
    {
        ...
        case 1: 	//commissionedunituid varchar
            ...
            break;
        case 2: 	//outputinwaterlist varchar
            {
                data_outputinwaterlist = src.get_outputinwaterlist();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_outputinwaterlist.c_str();
                bind[iCount].buffer_length = (unsigned long)data_outputinwaterlist.size();
            }
            break;
        case 3: 	//outputinairlist varchar
            {
                data_outputinairlist = src.get_outputinairlist();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_outputinairlist.c_str();
                bind[iCount].buffer_length = (unsigned long)data_outputinairlist.size();
            }
            break;
        case 4: 	//phantomagle float
            {
                data_phantomagle = src.get_phantomagle();
                bind[iCount].buffer_type = MYSQL_TYPE_FLOAT;
                bind[iCount].buffer = (void*)&data_phantomagle;
                bind[iCount].buffer_length = sizeof(float);
            }
            break;
        case 5: 	//scpdepth float
            {
                data_scpdepth = src.get_scpdepth();
                bind[iCount].buffer_type = MYSQL_TYPE_FLOAT;
                bind[iCount].buffer = (void*)&data_scpdepth;
                bind[iCount].buffer_length = sizeof(float);
            }
            break;
        case 6: 	//wedgefactordepth float
            {
                data_wedgefactordepth = src.get_wedgefactordepth();
                bind[iCount].buffer_type = MYSQL_TYPE_FLOAT;
                bind[iCount].buffer = (void*)&data_wedgefactordepth;
                bind[iCount].buffer_length = sizeof(float);
            }
            break;
        case 7: 	//crc varchar
	...
	DEL_ARRAY(bind);
	RtDatabaseObject* rtDatabaseObject = const_cast<RtMeasureddata*>(&src);
	rtDatabaseObject->setdirty_all_fields(false);
    return true;
}

bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtMeasureddata*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt)
    {
    ...
    //tmsmeasureddata    9 parameters
    const int field_num = MEASUREDDATA_FIELD_MAX;
    ...
    for (unsigned int i(0); i<column_count; ++i)
    {
        ...
        case 4: 	//phantomagle float
            ...
            bind[i].buffer= (char *)&data_phantomagle;
            break;
        case 5: 	//scpdepth float
            bind[i].buffer_type= MYSQL_TYPE_FLOAT;
            bind[i].buffer= (char *)&data_scpdepth;
            break;
        case 6: 	//wedgefactordepth float
            bind[i].buffer_type= MYSQL_TYPE_FLOAT;
            bind[i].buffer= (char *)&data_wedgefactordepth;
            break;
        case 8: 	//updatetime timestamp
    ...
    // Fetch all rows 
    int ret(1);
    RtMeasureddata* pDis = nullptr;
    while(true)
    {
        ret = mysql_stmt_fetch(stmt);
        if (ret!=0 && ret!=MYSQL_DATA_TRUNCATED) break;
        pDis = new RtMeasureddata();
        for (unsigned int i(0); i<column_count; ++i)
        {
            ...
            case 1: 	//commissionedunituid varchar
                ...
                break;
            case 2: 	//outputinwaterlist varchar
                pDis->set_outputinwaterlist((char *)bind[i].buffer);
                break;
            case 3: 	//outputinairlist varchar
                pDis->set_outputinairlist((char *)bind[i].buffer);
                break;
            case 4: 	//phantomagle float
                pDis->set_phantomagle(data_phantomagle);
                break;
            case 5: 	//scpdepth float
                pDis->set_scpdepth(data_scpdepth);
                break;
            case 6: 	//wedgefactordepth float
                pDis->set_wedgefactordepth(data_wedgefactordepth);
                break;
            case 7: 	//crc varchar
