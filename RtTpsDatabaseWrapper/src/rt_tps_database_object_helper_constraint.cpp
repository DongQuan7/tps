...
///
///  \file      rt_tps_database_object_helper_constraint.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtConstraint& src, bool bInsert)
{
    ...
    if (!bInsert)
    {
        for (unsigned int i(0); i < CONSTRAINT_FIELD_MAX; ++i)
        {
            if (!src.is_dirty(i)) continue;
            const char* file_name = ENUM2STRING_CONSTRAINT_FIELD[i];
            if (sName.empty())
            {
        ...
        sSql = "UPDATE tmsconstraint SET " + sName + " WHERE uid='" + src.get_uid() + "'";
    }
    else
    {
        std::string sValues("");
        ...
        sSql = "INSERT INTO tmsconstraint (" + sName + ") VALUES (" + sValues + ")";
    }
    ...
    for (unsigned int i(0); i < CONSTRAINT_FIELD_MAX; ++i)
    {
        ...
        case 0: 	//uid varchar
            ...
            break;
        case 1: 	//prescriptionuid varchar
            {
        ...
        case 7: 	//csg varchar
            {
                data_csg = src.get_csg();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_csg.c_str();
                bind[iCount].buffer_length = (unsigned long)data_csg.size();
            }
            break;
        case 8: 	//ingrid float
            {
	...
	DEL_ARRAY(bind);
	RtDatabaseObject* rtDatabaseObject = const_cast<RtConstraint*>(&src);
	rtDatabaseObject->setdirty_all_fields(false);
    return true;
}

bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtConstraint*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt)
    {
    ...
    //tmsconstraint    14 parameters
    const int field_num = CONSTRAINT_FIELD_MAX;
    ...
    // Fetch all rows 
    int ret(1);
    RtConstraint* pDis = nullptr;
    while(true)
    {
        ret = mysql_stmt_fetch(stmt);
        if (ret!=0 && ret!=MYSQL_DATA_TRUNCATED) break;
        pDis = new RtConstraint();
        for (unsigned int i(0); i<column_count; ++i)
        {
            ...
            case 6: 	//comments varchar
                ...
                break;
            case 7: 	//csg varchar
                pDis->set_csg((char *)bind[i].buffer);
                break;
            case 8: 	//ingrid float
