...
///
///  \file      rt_tps_database_object_helper_kernelweight.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtKernelweight& src, bool bInsert)
{
    ...
    if (!bInsert)
    {
        for (unsigned int i(0); i < KERNELWEIGHT_FIELD_MAX; ++i)
        {
            if (!src.is_dirty(i)) continue;
            const char* file_name = ENUM2STRING_KERNELWEIGHT_FIELD[i];
            if (sName.empty())
            {
        ...
        sSql = "UPDATE tmskernelweight SET " + sName + " WHERE uid='" + src.get_uid() + "'";
    }
    else
    {
        std::string sValues("");
        ...
        sSql = "INSERT INTO tmskernelweight (" + sName + ") VALUES (" + sValues + ")";
    }
    ...
    float		data_fieldsize = 0.f;
    std::string	data_kernelidlist = "";
    std::string	data_kernelweightlist = "";
    std::string	data_crc = "";
    ...
    for (unsigned int i(0); i < KERNELWEIGHT_FIELD_MAX; ++i)
    {
        ...
        case 1: 	//commissionedunituid varchar
            ...
            break;
        case 2: 	//fieldsize float
            {
                data_fieldsize = src.get_fieldsize();
                bind[iCount].buffer_type = MYSQL_TYPE_FLOAT;
                bind[iCount].buffer = (void*)&data_fieldsize;
                bind[iCount].buffer_length = sizeof(float);
            }
            break;
        case 3: 	//kernelidlist varchar
            {
                data_kernelidlist = src.get_kernelidlist();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_kernelidlist.c_str();
                bind[iCount].buffer_length = (unsigned long)data_kernelidlist.size();
            }
            break;
        case 4: 	//kernelweightlist varchar
            {
                data_kernelweightlist = src.get_kernelweightlist();
                bind[iCount].buffer_type = MYSQL_TYPE_VAR_STRING;
                bind[iCount].buffer = (void*)data_kernelweightlist.c_str();
                bind[iCount].buffer_length = (unsigned long)data_kernelweightlist.size();
            }
            break;
        case 5: 	//crc varchar
            {
	...
	DEL_ARRAY(bind);
	RtDatabaseObject* rtDatabaseObject = const_cast<RtKernelweight*>(&src);
	rtDatabaseObject->setdirty_all_fields(false);
    return true;
}

bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtKernelweight*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt)
    {
    ...
    //tmskernelweight    7 parameters
    const int field_num = KERNELWEIGHT_FIELD_MAX;
    ...
    for (unsigned int i(0); i<column_count; ++i)
    {
        ...
        case 2: 	//fieldsize float
            ...
            bind[i].buffer= (char *)&data_fieldsize;
            break;
        case 6: 	//updatetime timestamp
    ...
    // Fetch all rows 
    int ret(1);
    RtKernelweight* pDis = nullptr;
    while(true)
    {
        ret = mysql_stmt_fetch(stmt);
        if (ret!=0 && ret!=MYSQL_DATA_TRUNCATED) break;
        pDis = new RtKernelweight();
        for (unsigned int i(0); i<column_count; ++i)
        {
            ...
            case 1: 	//commissionedunituid varchar
                ...
                break;
            case 2: 	//fieldsize float
                pDis->set_fieldsize(data_fieldsize);
                break;
            case 3: 	//kernelidlist varchar
                pDis->set_kernelidlist((char *)bind[i].buffer);
                break;
            case 4: 	//kernelweightlist varchar
                pDis->set_kernelweightlist((char *)bind[i].buffer);
                break;
            case 5: 	//crc varchar
