...
///
///  \file      rt_tps_database_object_helper_monocompressedphasespacedata.cpp
...
bool RtDatabaseHelper::InsertUpdateDBFromObject(const RtMonocompressedphasespacedata& src, bool bInsert)
{
    ...
    if (!bInsert)
    {
        for (unsigned int i(0); i < MONOCOMPRESSEDPHASESPACEDATA_FIELD_MAX; ++i)
        {
            if (!src.is_dirty(i)) continue;
            const char* file_name = ENUM2STRING_MONOCOMPRESSEDPHASESPACEDATA_FIELD[i];
            if (sName.empty())
            {
        ...
        sSql = "UPDATE tmsmonocompressedphasespacedata SET " + sName + " WHERE uid='" + src.get_uid() + "'";
    }
    else
    {
        std::string sValues("");
        ...
        sSql = "INSERT INTO tmsmonocompressedphasespacedata (" + sName + ") VALUES (" + sValues + ")";
    }
    ...
    for (unsigned int i(0); i < MONOCOMPRESSEDPHASESPACEDATA_FIELD_MAX; ++i)
    {
        ...
        case 0: 	//uid varchar
            ...
            break;
        case 1: 	//injectelectronenergy float
            {
            ...
            break;
        case 2: 	//phspdistancefromtarget float
            {
	...
	DEL_ARRAY(bind);
	RtDatabaseObject* rtDatabaseObject = const_cast<RtMonocompressedphasespacedata*>(&src);
	rtDatabaseObject->setdirty_all_fields(false);
    return true;
}

bool RtDatabaseHelper::FetchDataFromDB(const std::string& sSql, std::vector<RtMonocompressedphasespacedata*>& vList)
{
    MYSQL_STMT* stmt = mysql_stmt_init(m_pMySql);
    if (!stmt)
    {
    ...
    //tmsmonocompressedphasespacedata    6 parameters
    const int field_num = MONOCOMPRESSEDPHASESPACEDATA_FIELD_MAX;
    ...
    // Fetch all rows 
    int ret(1);
    RtMonocompressedphasespacedata* pDis = nullptr;
    while(true)
    {
        ret = mysql_stmt_fetch(stmt);
        if (ret!=0 && ret!=MYSQL_DATA_TRUNCATED) break;
        pDis = new RtMonocompressedphasespacedata();
        for (unsigned int i(0); i<column_count; ++i)
        {
